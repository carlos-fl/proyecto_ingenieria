...
if (!$studentId || !$otherStudentId) {
    echo json_encode(new DataResponse("failure", error: new ErrorResponse(400, "Missing parameters")));
    return;
}
...
try {
    ...
    if ($conversation->num_rows == 0) {
        echo json_encode(new DataResponse("failure", error: new ErrorResponse(404, "No messages found between the users")));
        return;
    }
    ...
    echo json_encode(new DataResponse("success", $conversationData));
